  ...
  useEffect(() => {
    ...
    if (loadedFleet) {
      try {
        ...
        // Validating data from localStorage
        const validated = validateFleet(parsed);
        if (validated.length !== parsed.length) {
            console.warn("Some aircraft data in localStorage was invalid and filtered out.");
        }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const validated = validateFleet(json);

        if (validated.length > 0) {
          if (validated.length < json.length) {
              alert(`Imported ${validated.length} aircraft. ${json.length - validated.length} invalid items were skipped.`);
          } else {
          ...
          alert("Invalid file format: No valid aircraft data found.");
        }
      } catch (err) {
