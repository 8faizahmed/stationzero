...
export default function Home() {
  ...
  // --- HANDLERS (MEMOIZED) ---
  const handleUpdateCustom = useCallback((id: string, f: string, v: any) => {
    setCustomStations(prev => prev.map(s => s.id === id ? { ...s, [f]: v } : s));
  }, []);
  ...
  const handleAddCustom = useCallback(() => {
    setCustomStations(prev => [...prev, { id: `c-${Date.now()}`, name: "Item", weight: 0, arm: 0 }]);
  }, []);
  ...
  // --- CALCULATION LOGIC ---
  const isFlightPlanActive = toggles.flightPlan;
  ...
  const results = useMemo(() => {
    ...
    if (selectedPlane) {
      ...
      res = {
        ...
        isGo: isTakeoffInside && (isFlightPlanActive ? isLandingInside : true),
