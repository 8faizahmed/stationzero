...
import { validateFleetData } from "../utils/validation";
  ...
  useEffect(() => {
    ...
    if (loadedFleet) {
      try {
        ...
        const validated = validateFleetData(parsed);
        if (validated) {
          setSavedPlanes(validated);
        } else {
          console.error("Loaded fleet data validation failed or was empty.");
        }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const validated = validateFleetData(json);
